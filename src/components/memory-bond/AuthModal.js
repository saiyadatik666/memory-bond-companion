...
import { LogOut, X, Mail, Lock, User, ShieldCheck, Sparkles, Loader2, } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { supabase } from "@/integrations/supabase/client";
import { useI18n } from "@/lib/i18n";
export function AuthModal({ isOpen, onClose, store, }) {
    const { t } = useI18n();
    ...
    return (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs animate-in fade-in">
      <div className="relative w-full max-w-md rounded-3xl border-2 border-border bg-card p-6 sm:p-8 shadow-2xl space-y-5 animate-in zoom-in-95">
        {/* Close */}
        <button onClick={onClose} className="absolute right-5 top-5 rounded-full p-2 text-muted-foreground hover:bg-secondary hover:text-foreground">
          <X className="h-6 w-6"/>
        </button>
        ...
        <div className="rounded-2xl border border-dashed border-primary/40 bg-primary/5 p-3 space-y-2 text-center">
          ...
          <div className="grid grid-cols-2 gap-2">
            <Button type="button" variant="outline" size="sm" onClick={() => handleDemoSignIn("senior")} className="text-xs font-bold rounded-xl">
              👴 Senior User
            </Button>
            <Button type="button" variant="outline" size="sm" onClick={() => handleDemoSignIn("caregiver")} className="text-xs font-bold rounded-xl">
              👩‍⚕️ Caregiver
            </Button>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          {mode === "signup" && (<div>
              ...
              <div className="relative mt-1">
                ...
                <Input required value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="e.g. Ramesh Sharma" className="pl-9 rounded-xl"/>
              </div>
            </div>)}
          ...
          <div>
            ...
            <div className="relative mt-1">
              ...
              <Input required type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="name@example.com" className="pl-9 rounded-xl"/>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="relative mt-1">
              ...
              <Input required type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" className="pl-9 rounded-xl"/>
            </div>
          </div>
          ...
          {mode === "signup" && (<div>
              ...
              <div className="grid grid-cols-2 gap-2 mt-1">
                <button type="button" onClick={() => setRole("senior")} className={`p-2.5 rounded-xl border-2 font-bold text-xs transition-all ${role === "senior"
                ? "bg-primary text-primary-foreground border-primary"
                : "bg-secondary/40 border-border"}`}>
                  {t("iAmSenior") || "I am a Senior"}
                </button>
                <button type="button" onClick={() => setRole("caregiver")} className={`p-2.5 rounded-xl border-2 font-bold text-xs transition-all ${role === "caregiver"
                ? "bg-primary text-primary-foreground border-primary"
                : "bg-secondary/40 border-border"}`}>
          ...
          {errorMessage && (<p className="text-xs text-center text-destructive font-medium">{errorMessage}</p>)}

          {successMessage && (<p className="text-xs text-center text-success font-bold">{successMessage}</p>)}

          <Button type="submit" disabled={isLoading} className="w-full font-bold h-12 rounded-xl text-base">
            {isLoading ? (<Loader2 className="h-5 w-5 animate-spin"/>) : mode === "signin" ? (t("signIn") || "Sign In") : (t("signUp") || "Create Account")}
          </Button>
        </form>
        ...
        <div className="flex justify-between items-center text-xs pt-2 border-t border-border">
          ...
          <button type="button" onClick={handleSignOut} className="text-destructive font-bold hover:underline flex items-center gap-1">
