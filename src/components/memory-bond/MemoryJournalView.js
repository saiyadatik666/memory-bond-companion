...
import { BookOpen, Plus, Mic, Square, Play, Trash2, Calendar, Volume2, } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Textarea } from "@/components/ui/textarea";
import { speakText } from "@/lib/voiceParser";
export function MemoryJournalView({ store }) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="space-y-6 max-w-3xl mx-auto">
        {store.journal.length === 0 ? (<div className="rounded-3xl border border-dashed border-border bg-card p-12 text-center text-muted-foreground">
            No memories saved yet. Record a voice note or add your first cherished moment.
          </div>) : (store.journal.map((item, idx) => (<div key={item.id} className="relative rounded-3xl border-2 border-border bg-card p-6 shadow-sm space-y-4">
              <div className="flex items-start justify-between gap-4">
                ...
                year: "numeric",
                    ...
                    {item.kind === "voice" && (<span className="rounded-full bg-rose-500/20 text-rose-600 dark:text-rose-400 px-2 py-0.5 text-xs font-bold">
                        Voice Note
                      </span>)}
                  </div>
                ...
                <Button size="sm" variant="ghost" onClick={() => store.deleteJournalEntry(item.id)} className="text-destructive hover:bg-destructive/10 rounded-xl p-2">
                  <Trash2 className="h-4 w-4"/>
                </Button>
              </div>

              {item.body && (<p className="text-base text-foreground leading-relaxed font-medium">
                  {item.body}
                </p>)}

              {/* Voice Note Audio Playback */}
              {item.media_url ? (<div className="rounded-2xl bg-secondary/50 p-4 border border-border space-y-2">
                  <div className="flex items-center gap-2 text-sm font-bold text-foreground">
                    <Volume2 className="h-5 w-5 text-primary"/>
                    <span>Recorded Voice Message</span>
                  </div>
                  <audio controls src={item.media_url} className="w-full h-10"/>
                </div>) : item.kind === "voice" ? (<div className="rounded-2xl bg-primary/10 p-4 border border-primary/20 flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <Volume2 className="h-5 w-5 text-primary"/>
                    <span className="text-sm font-bold text-foreground">Voice Note Playback</span>
                  </div>
                  <Button size="sm" onClick={() => speakText(item.body || item.title)} className="gap-2 font-bold rounded-xl">
      ...
      {/* Add Memory Modal */}
      {isAddOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs overflow-y-auto">
          <div className="w-full max-w-lg rounded-3xl border-2 border-border bg-card p-6 sm:p-8 shadow-xl space-y-5 my-8 animate-in zoom-in-95">
            ...
            <form onSubmit={handleSaveMemory} className="space-y-4">
              <div>
                ...
                <Input required value={title} onChange={(e) => setTitle(e.target.value)} placeholder="e.g. Rongali Bihu celebration with children" className="rounded-xl mt-1"/>
              </div>

              <div>
                <Label>Memory Date</Label>
                <Input type="date" value={entryDate} onChange={(e) => setEntryDate(e.target.value)} className="rounded-xl mt-1"/>
              </div>

              <div>
                <Label>Story / Notes</Label>
                <Textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="Describe the wonderful feeling, people present, songs, or laughter..." className="rounded-xl mt-1" rows={4}/>
              </div>
              ...
              <div className="rounded-2xl border-2 border-dashed border-primary/40 bg-primary/5 p-4 text-center space-y-3">
                ...
                {isRecording ? (<div className="space-y-2">
                    ...
                    <div>
                      <Button type="button" variant="destructive" onClick={stopVoiceRecording} className="gap-2 font-bold rounded-xl">
                        <Square className="h-4 w-4"/> Stop Recording
                      </Button>
                    </div>
                  </div>) : (<div>
                    {recordedAudioUrl ? (<div className="space-y-2">
                        <p className="text-xs text-success font-bold">Voice note ready!</p>
                        <audio controls src={recordedAudioUrl} className="w-full h-8"/>
                        <Button type="button" variant="ghost" size="sm" onClick={() => setRecordedAudioUrl(null)} className="text-xs text-destructive">
                          Discard Audio
                        </Button>
                      </div>) : (<Button type="button" variant="outline" onClick={startVoiceRecording} className="gap-2 font-bold rounded-xl">
