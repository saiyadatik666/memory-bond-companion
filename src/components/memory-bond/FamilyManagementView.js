...
import { Users, Plus, Phone, Mail, Trash2, PhoneCall, } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { Switch } from "@/components/ui/switch";
export function FamilyManagementView({ store }) {
    ...
    const toggleEmergencyStatus = (id, currentStatus) => {
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {store.contacts.map((contact, idx) => (<div key={contact.id} className="rounded-3xl border-2 border-border bg-card p-6 shadow-sm space-y-4 flex flex-col justify-between">
            <div className="space-y-3">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-xl font-bold text-foreground flex items-center gap-2">
                      {contact.name}
                      {contact.is_emergency && (<span className="text-xs font-bold px-2.5 py-0.5 rounded-full bg-destructive/15 text-destructive">
                          Emergency SOS
                        </span>)}
                    </h3>
                ...
                <Button size="sm" variant="ghost" onClick={() => store.deleteContact(contact.id)} className="rounded-xl text-destructive hover:bg-destructive/10 p-2">
              ...
              <div className="space-y-2 pt-2 text-sm text-foreground">
                ...
                {contact.email && (<div className="flex items-center gap-2 text-muted-foreground">
              ...
              <div className="flex items-center justify-between pt-2 border-t border-border">
                ...
                <Switch checked={contact.is_emergency} onCheckedChange={() => toggleEmergencyStatus(contact.id, contact.is_emergency)}/>
              </div>
            </div>

            <div className="pt-3 border-t border-border flex justify-end gap-2">
              <a href={`tel:${contact.phone}`} className="inline-flex items-center gap-2 text-sm font-bold bg-secondary hover:bg-secondary/80 px-4 py-2.5 rounded-xl text-foreground transition-all">
      ...
      {/* Add Modal */}
      {isAddOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-xs">
          <div className="w-full max-w-md rounded-3xl border-2 border-border bg-card p-6 sm:p-8 shadow-xl space-y-4 animate-in zoom-in-95">
            ...
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                ...
                <Input required value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Sunita Sharma" className="rounded-xl mt-1"/>
              </div>

              <div>
                <Label>Relationship</Label>
                <Input value={relationship} onChange={(e) => setRelationship(e.target.value)} placeholder="Daughter, Son, Neighbor, Doctor" className="rounded-xl mt-1"/>
              </div>

              <div>
                <Label>Phone Number *</Label>
                <Input required type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+91 98765 43210" className="rounded-xl mt-1 font-mono"/>
              </div>

              <div>
                <Label>Email Address (Optional)</Label>
                <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="family@example.com" className="rounded-xl mt-1"/>
              </div>
